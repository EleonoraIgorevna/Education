...
// let l: {
//     [key: string]: number //любой ключ может содержать числовое значение
// } = {
//     key1: 1,
//     key2: 2,
//     key3: 3
...
// ОБОЗНАЧЕНИЕ ТИПОВ
// Запись выше не удобна и поэтому есть обозначение типов
// Мы можем заранее обьявить типы для нашего объекта
// Заранее определяем необходимые типы
// type someType = {
//     name: string
// }
...
// Второй раз someType применять нельзя
// Вариативность 
// type someType2 = string | number ;
// Вариативность интерфейсу присваивать нельзя
// Для простых типов удобнее type
// Для объектов и классрв interface удобнее

//Интерфейсы - другой способ типизации. Его преимущество в том, что мы можем расширять интерфейсы
interface bestObject
{
    key1: string; // в обьявлении типов ставим двоеточие ??
}
